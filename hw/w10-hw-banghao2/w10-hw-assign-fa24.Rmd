...
Look at the Normal Q-Q plot, I think the normality assumption for this model is violated, there seems to be fat tails within the plot.
...
The above is the reported observations' index that have high leverage.
...
The above is the reported observations' index that are influential.
...
- **Statistically**:
  ...
  - For SP test, since the p-value of the test is $0.4$, which is greater than most of the alpha value, we fail to reject the null hypothesis. This suggests that the assumption of residuals are normally distributed.
...
- Based on BP test, the Quadratic model is the best, since it's p-value of the BP test is the largest, suggesting biggest probability of constant variance of the noise.
- The p-value of the SP test for this model(Quadratic Model) is 0.3, which is greater than most of the common alpha values. Therefore, the normality assumption for this model is satisfied.
